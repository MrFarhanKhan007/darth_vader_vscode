...
export function getQuoteForContext(context: string): Quote {
  const matching = quotes.filter(q => q.contexts.includes(context));
  const pool = matching.length > 0 ? matching : quotes.filter(q =>
    q.contexts.includes("idle") || q.contexts.includes("welcome")
  );
